        ...
        async resolveIntentData(intent, context) {
            try {
                ...
                if (intent.name === 'user_management') {
                    ...
                    if (searchQuery) {
                        filteredUsers = allUsers.filter(user => user.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
...
server.addPlugin(createHealthMonitoringPlugin({
    checks: {
        ...
        componentServers: async () => {
            ...
            const results = await Promise.allSettled(servers.map(async (url) => {
                try {
...
//# sourceMappingURL=advanced-features-server.js.map